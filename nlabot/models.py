...
class Student(Base):
    ...
    id = Column(Integer, primary_key=True)
    user_id = Column(foreign_key=('users.id'))
    ...
    def __repr__(self):
        template = '<Student[{:d}] {:s} {:s}>'
        return template.format(self.id, self.first_name, self.last_name)
...
class Submission(Base):
    ...
    def __repr__(self):
        template = "<Submission[id={:d}] student name='{:s}' HW #'{:d}'" \
                   "attempt #'{:d}'>"
        return template.format(self.id, self.student_id, self.hw_id,
                self.submission_id)
